...
**第1层 P&L 终局**：毛利额是核心经营目标；毛利率、ROI、销售额、广告效率、库存、口碑等都是支撑毛利额达成的 KR（若子表为 P&L 类，数据中有则优先用于判断健康度）。这是复盘的顶视野。
...
**路径一 — 销售额/规模 KR 未达预期**（在规则零未覆盖或已排除断货主因时）：用「流量 × 转化 × 客单」思路拆解，并说明它对毛利额核心目标的影响。广告流量问题可联系展示/CTR/竞价与创意；自然侧若数据无法支撑排名，只写据现有数据能推出的结论。转化问题可核对 Buybox、价格、评分、退款等子表中是否可见的线索。
